...
const openk_version: std.SemanticVersion = .{ .major = 0, .minor = 1, .patch = 0 };

pub fn build(b: *std.Build) !void {
    const target = b.standardTargetOptions(.{});
    const optimize = b.standardOptimizeOption(.{});

    const mod = b.addModule("openk", .{
    ...
    const opt_version_string = b.option(
        []const u8,
        "version-string",
        "Override OpenK version string. Default is to find out with git.",
    );
