...
pub fn nodeMainToken(tree: *const Ast, node_index: Node.Index) TokenIndex {
    return tree.nodes.items(.main_token)[@intFromEnum(node_index)];
}

pub fn nodeData(tree: *const Ast, node_index: Node.Index) Node.Data {
...
pub fn lastToken(tree: Ast, node: Node.Index) TokenIndex {
    ...
    while (true) switch (tree.nodeTag(n)) {
        ...
        .apply_binary,
        => n = tree.nodeData(n).node_and_opt_node[1].unwrap() orelse return tree.nodeMainToken(n) + end_offset,
...
pub const Node = struct {
    ...
    pub const OptionalOffset = enum(u32) {
        zero = 0,
        none = std.math.maxInt(u32),
        _,
    ...
    pub const Tag = enum {
        ...
        /// The `main_token` field is the system command.
        system,
    };
